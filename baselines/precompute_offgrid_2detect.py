"""Precomputes 2DeteCT test states at doses outside the training grid."""
...
T_LIST = [0.1, 0.3, 0.5, 0.7, 0.9, 1.1, 1.25]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--sino_path', default=f"{DATA_ROOT}/2DeteCT_raw")
    ap.add_argument('--out', default=f"{DATA_ROOT}/2DeteCT_dose_bridge_offgrid/precomputed")
    ap.add_argument('--I0_high', type=float, default=1e5)
    ap.add_argument('--alpha', type=float, default=0.01)
    ap.add_argument('--seed', type=int, default=777)
    ap.add_argument('--n_slices', type=int, default=1000)
    args = ap.parse_args()
    config = T.Config()
    config.sino_path = args.sino_path
    config.I0_high = args.I0_high
    config.alpha = args.alpha
    config.I0_low = args.I0_high * args.alpha
    os.makedirs(args.out, exist_ok=True)
    projector = T.AstraProjector2D(config)
    sids = [f'slice{i:05d}' for i in range(1, args.n_slices + 1)]
    test = split_slice_ids(sids, '2detect', 'test')
    print(f"off-grid precompute: {len(test)} test slices, t={T_LIST}, out={args.out}", flush=True)
    for n, sid in enumerate(sorted(test)):
        slice_idx = int(sid[5:])
        if all(os.path.exists(os.path.join(args.out, f"{sid}_step{k}.npz")) for k in range(len(T_LIST))):
            continue
        try:
            ...
            vmin, vmax = float(x0_raw.min()), float(x0_raw.max())
            ...
            for k, t in enumerate(T_LIST):
                ...
                np.savez_compressed(os.path.join(args.out, f"{sid}_step{k}.npz"),
                                    x_t=xt, x0=x0, t=np.float32(t), slice_idx=slice_idx,
                                    vmin=np.float32(vmin), vmax=np.float32(vmax), dose_frac=np.float32(a_t))
            if n % 20 == 0:
                print(f"  {n}/{len(test)} {sid} done", flush=True)
